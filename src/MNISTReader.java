...
public class MNISTReader {
	...
	public static int[] getLabels(String infile) {
		...
		int centerX = (int) (Math.ceil((right - left) / 2)) + left + 1;
		int centerY = (int) (Math.ceil((bottom - top) / 2)) + top + 1;
	...
	private static float[] readImage(int numRows, int numCols, ByteBuffer bb) {
		float[] image = new float[numRows * numCols];
		for (int pix = 0; pix < (numRows * numCols); pix++)
			image[pix] = bb.get() & 0xFF;
		return image;
	}
